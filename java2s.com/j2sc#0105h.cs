// j2sc#0105h.cs: Ýçiçe armalý aduzamlarý tiplemesi örneði.

using System;
using S1 = Sayaç1;
using S2 = Sayaç1.Sayaç2;
using S3 = Sayaç1.Sayaç2.Sayaç3;
namespace Sayaç1 {
    class Sayacým {
        private int sayaç = 2023;
        public void Sayaç() {Console.WriteLine ("Sayaç1: {0}", --sayaç);}
    }
    namespace Sayaç2 {
        class Sayacým {
            private int sayaç = 2023;
            public void Sayaç() {Console.WriteLine ("Sayaç2: {0}", ++sayaç);}
        }
        namespace Sayaç3 {
            class Sayacým {
                private int sayaç = 1957;
                public void Sayaç() {Console.WriteLine ("Sayaç3: {0}", ++sayaç);}
            }
        }

    }
}
namespace DilTemelleri {
    class Aduzam8 {
        static void Main() {
            Console.Write ("Ýçiçe armalý aduzam tipmemesinde hiyerarþi dikkatle takip edilmelidir.\nTuþ...");Console.ReadKey();Console.WriteLine ("\n");

            var say1 = new S1::Sayacým();
            var say2 = new S2::Sayacým();
            var say3 = new S3::Sayacým();
            for (int i=0; i < 5; i++) {say1.Sayaç(); say2.Sayaç(); say3.Sayaç();}

            Console.Write ("\nTuþ..."); Console.ReadKey();
        }
    } 
}